...
- **Detalhamento de tarefas por dia** (botão "Tarefas" na célula de Descrição): registre o horário exato (início/fim) de cada atividade dentro da sua jornada, com duração calculada automaticamente. Cada tarefa pode receber **uma ou mais tags coloridas** salvas (ex.: "Reunião", "Urgente") — cada tag vira uma etiqueta ao lado da tarefa, sem sobrescrever o que você digitou, e dá pra criar uma tarefa já marcada com um clique direto na tag (sem precisar clicar em "Adicionar tarefa" antes). Um gerenciador de tags permite criar (com nome + cor, escolhida numa paletinha) e excluir tags a qualquer momento. O detalhamento aparece também no Excel, no PDF e na planilha do Google Sheets.
