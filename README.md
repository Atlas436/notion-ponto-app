...
- **Detalhamento de tarefas por dia** (botão "Tarefas" na célula de Descrição): registre o horário exato (início/fim) de cada atividade dentro da sua jornada, com duração calculada automaticamente. Cada tarefa tem um seletor pra escolher uma "tag" salva (ex.: "Reunião") sem redigitar — a lista de tags fica guardada e cada tarefa recebe só a que você escolher para ela. O detalhamento aparece também no Excel, no PDF e na planilha do Google Sheets.
